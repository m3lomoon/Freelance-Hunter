...
| **痛點** | 市面冥想 App 是固定模板音檔，聽三天就膩；別人的聲音對潛意識說服力有限；沒有人問「你現在卡在哪」；也沒有人證明「有沒有效」 |
| **解法** | ① 先問「最近想改善什麼」② **13 種真實場合**的冥想（開會前、約會前、發文前…）＋ 6 種呼吸節奏 ③ AI 讀你的狀態與日記動態寫引導詞 ④ 用你錄的 30 秒聲音複製，**由你唸給你聽** ⑤ **前後打分**，用數據證明有效 |
...
| **留存？** | 四個 Hook：心情打卡（3 秒）→ 情境冥想（60 秒，錨定在真實事件上，不是「每天記得打開」）→ 感恩日記（90 秒）→ 睡前導引（床上）。**前後評分**讓用戶看見自己的數據，這是最強的續訂理由。目標 D30 留存 ≥ 25%（Calm 約 20%） |
| **只是把 Calm 翻成中文？** | Calm 是「內容庫」，我們是「**觸發器**」。使用者不是想冥想，是**十分鐘後要進會議室**。把冥想綁在事件上（開會、約會、發文、談錢），打開率完全不同；而且這些場合是亞洲職場與社群文化特有的 |
...
**為什麼敢收 290**：情境庫 + 你的聲音 + 效果數據，三個都是模板音檔給不了的。
...
2. **內容飛輪**：每週一支情境短片（「約會前我都聽這個」「發文前的 60 秒」），用自己的聲音；片尾 CTA 進 App。情境本身就是可傳播的內容單位
...
- **內容同質化**：語氣人格化（溫柔／堅定／Baddie）＋ 情境庫是品牌資產，持續加入創辦人 IP 專屬語氣
- **效果宣稱**：前後評分只呈現「使用者自評變化」，不做療效宣稱，文案避開「治療、改善病症」等字眼
