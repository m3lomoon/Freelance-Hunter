...
把一支長 podcast 影片，自動剪成好幾支**帶中英雙語字幕的短片**，預設就是 **IG Reels 規格**（1080x1920、9:16）。
...
- 上排原文、下排翻譯，兩排都燒錄進畫面（不是外掛字幕）。
- **中文一律是繁體中文＋台灣用語**（用 OpenCC `s2twp` 轉換），不管是 whisper 聽打出來的簡體、還是 Claude 翻譯出來的中文，都會強制轉換，不會出現「軟件、網絡」這種中國用語。
- 字幕位置有避開 **IG Reels 介面**：底部留了約 270-380px 高度（避開文字說明／音樂資訊／頭像＋按鈕），左右留了 90px（避開右側直排的讚/留言/分享按鈕）。這些數字在 `settings.py` 的 `SUB_MARGIN_*` 可以再微調。
